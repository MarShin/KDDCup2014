...
pipeline = Pipeline([
('tfidf_vectorizer', TfidfVectorizer(max_df=0.99, min_df=0.01, ngram_range=(1, 2), sublinear_tf=True)),
('classifier', SGDClassifier(penalty="l2",loss="log", shuffle=True,n_iter=20, alpha=0.000005))])
...
for i in ( 'short_description','need_statement', 'essay'):
    ...
    essays[i+'_proba'] = pipeline.predict_proba(essays[i])[:,1]

#Assign prediction values for each data as features
predictions = pd.concat([essays['projectid'],essays['short_description_proba'],essays['need_statement_proba'],essays['essay_proba']], axis=1)
predictions.to_csv('text_predictions.csv',index=False)
